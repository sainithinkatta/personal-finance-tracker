...
const BankAccountForm: React.FC<BankAccountFormProps> = ({
  ...
  const [availableBalance, setAvailableBalance] = useState(
    account?.available_balance?.toString() || ""
  );
  const [paymentDueDate, setPaymentDueDate] = useState(
    account?.payment_due_date?.toString() || ""
  );
  ...
  const computedDueBalance =
    accountType === "Credit" && creditLimit && availableBalance
    ...
    if (accountType === "Credit" && creditLimit && availableBalance) {
      if (Number(availableBalance) > Number(creditLimit)) {
        setAvailableBalanceError(
          "Available balance cannot exceed credit limit"
        );
      } else {
        setAvailableBalanceError("");
      }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (accountType === "Credit") {
      if (
        ...
        isNaN(Number(paymentDueDate))
      ) {
        toast({
          title: "Invalid Input",
          description:
            "Please enter valid credit limit, available balance and payment due date for credit accounts",
          variant: "destructive",
        });
        return;
      }
          ...
          {creditLimit && availableBalance && (
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-600">
                Computed Due Balance (Read-only)
              </label>
