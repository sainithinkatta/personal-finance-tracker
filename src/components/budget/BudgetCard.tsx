...
export const BudgetCard: React.FC<BudgetCardProps> = ({
  ...
  const CategoryItem = ({
    ...
    const allocated =
      (budget[
        `${category.toLowerCase()}_allocated` as keyof Budget
      ] as number) || 0;
    const spent =
      (budget[`${category.toLowerCase()}_spent` as keyof Budget] as number) ||
      0;
      ...
      <div className="group relative">
        <div className="flex items-center justify-between p-3 rounded-lg bg-white border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all duration-200">
          <div className="flex items-center space-x-2">
            <div
              className={`p-1.5 rounded-md ${config.bg} ${config.color} ring-1 ${config.ring}`}
            >
            ...
            <div>
              ...
              <div className="text-xs text-gray-500">
                ...
                {formatCurrency(allocated, budget.currency)}
              </div>
            </div>
          </div>
          <div className="text-right">
            <div
              className={`text-xs font-bold ${
                categoryProgress > 100 ? "text-red-600" : "text-gray-900"
              }`}
            >
              ...
              <div className="flex items-start space-x-2">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-amber-700 mt-0.5">
                    You have{" "}
                    {formatCurrency(
                      budget.total_amount - totalAllocated,
                      budget.currency
                    )}{" "}
                    unallocated. Click the settings button to distribute funds
                    across categories.
                  </div>
                </div>
              </div>
